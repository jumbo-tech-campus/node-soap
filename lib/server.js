...
Server.prototype._executeMethod = function(options, callback) {
  ...
  function handleResult(result) {
    ...
    if (style === 'rpc') {
      resBody = self.wsdl.objectToRpcXML(outputName, resBody, '', self.wsdl.definitions.$targetNamespace);
    } else {
      var element = self.wsdl.definitions.services[serviceName].ports[portName].binding.methods[methodName].output;
      resBody = self.wsdl.objectToDocumentXML(outputName, resBody, element.targetNSAlias, element.targetNamespace);
      if (resHeaders && resHeaders.length) {
        resHeaders = js2xmlparser("soap:Header", resHeaders[0], {
          attributeString: '$',
          declaration: {
            include: false
          }
    ...
    callback(self._envelope(resBody, resHeaders));
  }
